...
**Smart caching** reduces API calls by checking Vue Query cache first:
...
<template>
	<button @click="handleUpdate(1)">Update Post</button>
</template>
```
...
const isLoading = computed(
	() => isLoadingUser.value || isLoadingProjects.value,
);
</script>
